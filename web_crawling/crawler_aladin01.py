...
div_list = soup.find_all('div', class_='ss_book_box') # 파이썬 클래스 _ 

# print('div_list에 들어있는 데이터수 :', len(div_list))
# print(div_list[0]) # 1위 책만 가져와보자

# li 안에 우리가 필요로 하는 텍스트가 존재.
# 2,3,4 번째 li의 텍스트를 가져와야 하겠더라.
...
# 텍스트만을 추출하여 반환합니다.
book_title = first_book[1].text  # 두번째 li 를 지목하기 위해 1번 인덱스로 지목
